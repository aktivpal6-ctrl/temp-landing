...
import { IMAGES, MATCH_CRITERIA, ACTIVITIES, SAFETY, STEPS, COMMUNITY_TAGS } from "../data/survey";
    ...
    <section ref={ref} className="relative min-h-[100svh] overflow-hidden bg-[#0F291E] grain" data-testid="hero">
      ...
      <motion.div style={{ y: yContent }} className="relative z-10 max-w-6xl mx-auto px-6 pt-8 pb-24 min-h-[100svh] flex flex-col">
        ...
        <motion.div style={{ opacity: fade }} className="flex-1 flex flex-col justify-center max-w-4xl mt-16">
          ...
          >
            <span className="w-8 h-px bg-[#FF5C00]" /> Life happens when you move
          </motion.span>
          ...
          >
            Go for the hike. Run that trail. Ski that mountain. Explore somewhere new.
          </motion.p>
          <motion.p
          ...
          >
            You don't always need a plan. Sometimes, you just need the right people. AKTIVPAL helps you find them.
          </motion.p>
          ...
          >
            ...
            <span className="text-sm text-[#F7F7F2]/50 font-medium">Starting in British Columbia</span>
          </motion.div>
        </motion.div>
      </motion.div>
...
export const Problem = () => {
  ...
  return (
    <section ref={ref} className="relative bg-[#0F291E] py-28 md:py-40 grain overflow-hidden" data-testid="problem">
      <DriftNumeral n="01" y={y} dark className="text-[24rem] -right-12 top-2" />
      <div className="relative z-10 max-w-4xl mx-auto px-6">
        <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#FF5C00]">01 — More than a workout</span>
        <Reveal delay={0.05}>
        ...
        <Reveal delay={0.1}>
          <p className="mt-8 text-xl md:text-2xl text-[#F7F7F2]/70 max-w-2xl leading-relaxed">
            It's about the places you discover, the people you meet, and the stories you create along the way.
          </p>
        </Reveal>
        <Reveal delay={0.15}>
          <p className="mt-6 text-2xl md:text-3xl font-display font-bold text-[#F7F7F2]">
            But finding someone who actually wants to do it with you? <span className="text-[#FF5C00]">That's the hard part.</span>
          </p>
        </Reveal>
        <Reveal delay={0.2}>
          <p className="mt-6 text-lg text-[#F7F7F2]/60">AKTIVPAL makes it easier.</p>
        </Reveal>
      </div>
    </section>
  );
  ...
  return (
    <section ref={ref} className="relative bg-[#F7F7F2] py-28 md:py-36 overflow-hidden" data-testid="match-criteria">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        <Reveal>
          <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#FF5C00]">02 — Find someone who's up for it</span>
          ...
          <p className="mt-5 text-lg text-[#4A524A] max-w-xl">
            Hike this weekend? A running partner? A ski day? Match, then get out there.
          </p>
        </Reveal>
        <motion.div
        ...
        >
          {MATCH_CRITERIA.map((c, i) => (
            ...
            >
              ...
              <h3 className="font-display font-bold text-lg md:text-xl text-[#0F291E]">{c.title}</h3>
              <p className="mt-1.5 text-sm text-[#4A524A] leading-relaxed">{c.body}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
...
export const HowItWorks = () => {
  ...
  return (
    <section ref={ref} className="relative bg-[#0F291E] py-28 md:py-36 grain overflow-hidden" data-testid="how-it-works">
      <DriftNumeral n="03" y={y} dark className="text-[24rem] -right-14 top-6" />
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <Reveal>
          <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#FF5C00]">03 — How AKTIVPAL works</span>
        ...
        >
          {STEPS.map((s, i) => (
            <motion.div
              key={s.n} variants={item} whileHover={{ y: -6 }}
              transition={{ type: "spring", stiffness: 300, damping: 20 }}
              className="rounded-3xl border border-white/12 p-8 bg-white/[0.03]"
              data-testid={`step-card-${i}`}
            >
              <span className="font-display font-black text-5xl text-[#FF5C00]/40">{s.n}</span>
              <h3 className="mt-4 font-display font-bold text-xl text-[#F7F7F2]">{s.title}</h3>
              <p className="mt-2 text-[15px] text-[#F7F7F2]/65 leading-relaxed">{s.body}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
...
export const ProductMoment = () => {
  ...
  const paras = [
    { o: o1, y: ty1, cls: "text-[#F7F7F2]/75",
      node: <>"We should go hiking sometime." "Let's run together." And then… nothing happens.</> },
    { o: o2, y: ty2, cls: "text-[#F7F7F2] font-medium",
      node: <>AKTIVPAL turns "sometime" into "this weekend." <span className="text-[#FF5C00] font-semibold">Find the people. Make the plan. Go.</span></> },
  ];

  return (
    <section ref={sectionRef} className="relative bg-[#F7F7F2] py-20 md:py-24" data-testid="product-moment">
      <div className="max-w-6xl mx-auto px-6 relative z-10 grid lg:grid-cols-2 gap-14 items-start lg:min-h-[120vh]">
        <div className="lg:sticky lg:top-28 lg:h-[80vh] flex flex-col justify-center">
          <Reveal>
            <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#FF5C00]">04 — We've all said it</span>
          ...
          <div className="mt-8 space-y-5 text-lg leading-relaxed">
            {paras.map((p, i) => (
              <motion.p key={i} style={reduce ? undefined : { opacity: p.o, y: p.y }} className={p.cls.replace("text-[#F7F7F2]/75", "text-[#4A524A]").replace("text-[#F7F7F2] font-medium", "text-[#1A1D1A] font-medium")} data-testid={`imagine-line-${i}`}>
                {p.node}
              </motion.p>
            ))}
          </div>
...
export const Community = () => {
  const { ref, y } = useDrift(80);
  return (
    <section ref={ref} className="relative bg-[#0F291E] py-28 md:py-36 grain overflow-hidden" data-testid="community">
      ...
      <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
        <Reveal>
          <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#FF5C00]">05 — More than partners</span>
        ...
        <Reveal delay={0.1}>
          <p className="mt-12 text-xl md:text-2xl font-display font-bold text-[#F7F7F2]">
            The best part of an adventure… <span className="text-[#FF5C00]">is who you share it with.</span>
          </p>
        </Reveal>
      </div>
    </section>
...
export const ActivitiesGrid = () => {
  ...
  return (
    <section ref={ref} className="relative bg-[#F7F7F2] py-28 md:py-36 overflow-hidden" data-testid="activities">
      <DriftNumeral n="06" y={y} className="text-[24rem] -right-16 top-0" />
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <Reveal>
          <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#FF5C00]">06 — Move your way</span>
          <h2 className="mt-4 font-display font-extrabold text-3xl md:text-5xl tracking-tight text-[#1A1D1A] max-w-2xl">
        ...
        >
          {ACTIVITIES.map((a, i) => (
            ...
            >
              ...
              <h3 className="font-display font-bold text-base md:text-lg text-[#0F291E]">{a.title}</h3>
              <p className="mt-1 text-sm text-[#4A524A] leading-snug">{a.body}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
...
export const Safety = () => {
  ...
  return (
    <section ref={ref} className="relative bg-[#0F291E] py-28 md:py-36 grain overflow-hidden" data-testid="safety">
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <Reveal>
          <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#FF5C00]">07 — Built for trust</span>
        ...
        >
          {SAFETY.map((c, i) => (
            ...
            >
              ...
              <h3 className="font-display font-bold text-base text-[#F7F7F2]">{c.title}</h3>
              <p className="mt-1.5 text-sm text-[#F7F7F2]/60 leading-relaxed">{c.body}</p>
            </motion.div>
          ))}
        </motion.div>
        <Reveal delay={0.1}>
          <p className="mt-14 text-lg md:text-xl text-[#F7F7F2]/80 max-w-3xl leading-relaxed border-l-4 border-[#FF5C00] pl-6">
            Unlike dating apps, the goal is simple: <strong className="font-display font-extrabold text-[#F7F7F2]">find people who want to do something.</strong>
          </p>
        </Reveal>
      </div>
    </section>
...
export const FinalCTA = () => {
  ...
  return (
    <section ref={ref} className="relative bg-[#F7F7F2] py-28 md:py-40 overflow-hidden" data-testid="final-cta">
      ...
      <DriftNumeral n="08" y={y} className="text-[22rem] left-1/2 -translate-x-1/2 top-4" />
      <div className="max-w-3xl mx-auto px-6 text-center relative z-10">
        ...
        <Reveal delay={0.1}>
          <p className="mt-8 text-lg md:text-xl text-[#4A524A] max-w-xl mx-auto leading-relaxed">
            A hiking partner. A running buddy. Someone to explore a new mountain with. Or a friend you never would have met otherwise.
          </p>
        </Reveal>
        <Reveal delay={0.15}>
          <p className="mt-6 font-display font-black text-2xl md:text-3xl text-[#0F291E]">
            Find your people. Move more. <span className="text-[#FF5C00]">Experience more.</span>
          </p>
        </Reveal>
        <Reveal delay={0.2} className="mt-10">
          <CTA testId="finalcta-cta" onClick={scrollToSurvey}>JOIN AKTIVPAL <Ic name="ArrowRight" size={18} /></CTA>
          <p className="mt-4 text-sm text-[#4A524A]">Starting in British Columbia. Built to connect people everywhere.</p>
        </Reveal>
      </div>
    </section>
  );
  ...
  <footer className="relative bg-[#0F291E] grain overflow-hidden" data-testid="footer">
    <div className="max-w-5xl mx-auto px-6 py-24 md:py-28 text-center relative z-10">
      <Reveal>
        ...
        <p className="mt-4 text-[#F7F7F2]/60 text-lg">Starting in British Columbia. Built to connect people everywhere.</p>
      </Reveal>
      {!converted && (
        <Reveal delay={0.1} className="mt-10">
          <CTA testId="footer-cta" onClick={scrollToSurvey}>JOIN THE MOVEMENT <Ic name="ArrowRight" size={18} /></CTA>
