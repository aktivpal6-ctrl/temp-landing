...
const Ic = ({ name, ...p }) => { const C = Icons[name] || Icons.Circle; return <C {...p} />; };
...
const ChoiceCard = ({ label, selected, onClick, testId }) => (
  <motion.button
    ...
    className={`w-full text-left flex items-center gap-4 p-5 rounded-2xl border-2 transition-colors ${
      selected ? "border-[#FF5C00] bg-[#FF5C00]/10" : "border-black/10 bg-white hover:border-[#FF5C00]/40 hover:bg-[#FF5C00]/[0.04]"
    }`}
  >
    ...
    <span className={`font-semibold text-[15px] ${selected ? "text-[#0F291E]" : "text-[#1A1D1A]"}`}>{label}</span>
  </motion.button>
);
...
const Scale = ({ value, low, high, onChange, testId }) => (
  <div className="mt-2">
    <div className="flex items-center justify-between gap-2">
      {[1, 2, 3, 4, 5].map((n) => (
        <motion.button
          key={n} type="button" data-testid={`${testId}-${n}`} onClick={() => onChange(n)}
          ...
          className={`flex-1 aspect-square max-w-[72px] rounded-2xl border-2 font-display font-black text-xl md:text-2xl flex items-center justify-center transition-colors ${
            value === n ? "border-[#FF5C00] bg-[#FF5C00] text-white" : "border-black/10 bg-white text-[#0F291E] hover:border-[#FF5C00]/40"
          }`}
        >
          {n}
        </motion.button>
...
export const Survey = ({ onComplete, standalone = false }) => {
  ...
  const [contact, setContact] = useState({ name: "", email: "", instagram: "" });
  ...
  const setAns = useCallback((id, val) => setAnswers((a) => ({ ...a, [id]: val })), []);
  ...
  const back = () => { if (idx > 0) { setDir(-1); setIdx((i) => i - 1); } };

  const submit = async () => {
    setSubmitting(true);
    setError(false);
    const payload = {
      answers: SURVEY.filter((s) => buildSteps(answers).find((v) => v.id === s.id)).map((s) => {
      ...
      contact: answers.q15 === "Yes — I'd love to" ? contact : null,
    };
    // include q6 followup
    if (answers.q6b) payload.answers.push({ question: SURVEY.find((x) => x.id === "q6").followup.label, answer: answers.q6b });
    try {
  ...
  const showContact = q?.reveal && answers[q.id] === q.reveal.on;
  ...
  const resetSurvey = () => {
    ...
    setAnswers({});
    setOthers({});
    setContact({ name: "", email: "", instagram: "" });
    setIdx(0);
    setDir(1);
    setSubmitting(false);
    setDone(false);
    ...
    <section id="survey" className={sectionClasses} data-testid="survey">
      <div className={containerClasses}>
        ...
        {!started && !done && (
          ...
          >
            <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#FF5C00]">Help Build AKTIVPAL</span>
            ...
            <p className="mt-8 text-[15px] md:text-base text-[#4A524A] leading-relaxed max-w-xl mx-auto">
              We're building AKTIVPAL to help people find the right people to be active with—whether that's a hiking
              partner, running buddy, ski partner, cycling group, climbing partner, or simply someone who wants to get
              outside. Your answers will help us understand whether this is a problem worth solving—and what AKTIVPAL
              should actually become.
            </p>
            {standalone && (
              <p className="mt-4 text-sm font-semibold text-[#0F291E]" data-testid="survey-page-intro-note">
                You can take the full survey here without leaving the rest of the landing page in the background.
              </p>
            )}
            <div className="mt-10">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-bold uppercase tracking-widest text-[#4A524A]" data-testid="survey-progress-label">
                  Question {idx + 1} of {total}
                </span>
                <span className="text-xs font-bold text-[#FF5C00]">{progress}%</span>
              </div>
            ...
            <div className="relative min-h-[360px]">
              <AnimatePresence mode="wait" custom={dir}>
                <motion.div
                  key={q.id} custom={dir} variants={variants}
                ...
                >
                  {q.section && (
                    <div className="mb-4 rounded-xl bg-[#0F291E] text-[#F7F7F2] px-4 py-3 text-sm font-semibold flex items-center gap-2">
                      <Ic name="ShieldCheck" size={16} className="text-[#FF5C00]" /> {q.section}
                    </div>
                  )}
                  <h3 className="font-display font-bold text-2xl md:text-3xl tracking-tight text-[#0F291E] leading-tight">
                    {q.label}
                  </h3>
                  {q.hint && <p className="mt-2 text-sm font-semibold text-[#FF5C00]">{q.hint}</p>}
                  ...
                  >
                    ...
                    {q.type === "single" && q.options.map((opt) => (
                      <motion.div key={opt} variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
                        <ChoiceCard
                          label={opt} selected={answers[q.id] === opt}
                    ...
                    {q.type === "scale" && (
                      <Scale
                        value={answers[q.id]} low={q.low} high={q.high}
                        onChange={(n) => setAns(q.id, n)} testId={`survey-scale-${q.id}`}
                      />
                    )}
                    ...
                    {q.followup && (
                      <div className="pt-2">
                        <label className="block text-sm font-bold text-[#0F291E] mb-2">{q.followup.label}</label>
                        <textarea
                          data-testid="survey-text-q6b" rows={3}
                          value={answers.q6b || ""} onChange={(e) => setAns("q6b", e.target.value)}
                    ...
                    <AnimatePresence>
                      {showContact && (
                        <motion.div
                          ...
                          exit={{ opacity: 0, height: 0 }} className="space-y-3 pt-3 overflow-hidden"
                        >
                          <p className="text-sm font-bold text-[#0F291E]">Amazing — where can we reach you?</p>
                          {[
                            { k: "name", ph: "Name" },
                            { k: "email", ph: "Email" },
                            { k: "instagram", ph: "Instagram (optional)" },
                          ].map((f) => (
                            <input
                              key={f.k} data-testid={`contact-${f.k}`} placeholder={f.ph}
                              value={contact[f.k]} onChange={(e) => setContact((c) => ({ ...c, [f.k]: e.target.value }))}
                              className="w-full p-4 rounded-2xl border-2 border-black/10 bg-white focus:border-[#FF5C00] outline-none text-[15px]"
                            />
                          ))}
                        </motion.div>
            ...
            {/* nav */}
            {error && (
              <p className="mt-4 text-sm font-semibold text-red-600 text-center" data-testid="survey-error">
                Something went wrong sending your answers. Please tap Submit to try again.
              </p>
            )}
            <div className="mt-8 flex items-center justify-between gap-4">
              <button
                data-testid="survey-back-btn" onClick={back} disabled={idx === 0}
                className={`inline-flex items-center gap-1.5 text-sm font-bold transition-colors ${
                  idx === 0 ? "text-black/20 cursor-not-allowed" : "text-[#0F291E] hover:text-[#FF5C00]"
                }`}
              >
              ...
              >
                {submitting ? "Sending…" : idx === total - 1 ? "Submit" : "Next"}
                {!submitting && <Ic name={idx === total - 1 ? "Send" : "ArrowRight"} size={16} />}
              </CTA>
            </div>
          </motion.div>
        )}
        ...
        {done && (
          <motion.div
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center"
          ...
          >
            ...
            >
              <motion.svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
            ...
            <p className="mt-5 text-lg text-[#4A524A] max-w-lg mx-auto leading-relaxed">
              Thanks for helping us build AKTIVPAL. We're starting in Canada with a simple idea: you shouldn't have to
              choose between staying home and going alone.
            </p>
            <p className="mt-4 text-[15px] text-[#4A524A] max-w-lg mx-auto">
              We'll use what we learn from this community to shape the first version of AKTIVPAL.
            </p>
            <p className="mt-8 font-display font-black text-2xl md:text-3xl text-[#0F291E]">
              You're one step closer to finding your <span className="text-[#FF5C00]">AKTIVPAL.</span>
            </p>
